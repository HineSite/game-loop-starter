export class GameLoop {
    ...
    constructor(onUpdate, onDraw, onStart, onStop, onPause, onResume, targetFps) {
    ...
    #onRun() {
        ...
        if (this.#loopState === GameLoop.LoopState.RUNNING) {
            if (typeof this.#onUpdateCallback === 'function') {
                delay = this.#adjustForActualDelay ? actualLoopDelay : this.#targetLoopInterval;
                this.#onUpdateCallback(delay, (delay / 1000));
            }
        }
        else if (this.#loopState ===  GameLoop.LoopState.PAUSED) {
            if (typeof this.#pausedCallback === 'function') {
                delay = this.#adjustForActualDelay ? actualLoopDelay : (this.#pausedInterval ?? this.#targetLoopInterval);
                this.#pausedCallback(delay, (delay / 1000));
            }
        }

        if (typeof this.#onDrawCallback === 'function') {
            this.#onDrawCallback(delay, (delay / 1000));
        }
    ...
    #log(type, title, message) {
        if (this.#logger) {
            if (this.#errorLogs === false && type === GameLoop.LogType.ERROR ||
                this.#warnLogs === false && type === GameLoop.LogType.WARN ||
                this.#debugLogs === false && type === GameLoop.LogType.DEBUG ||
                this.#infoLogs === false && type === GameLoop.LogType.INFO) {
    ...
    enableLogging(error, warn, debug, info) {
